 ...
 * nearest integer BDT. Mirrors the backend PricingService exactly, so the
 * client can live-preview the ladder without a round-trip.
 */
export function computeStage(prevPrice: number, input: StageInput): StageResult {
...
export function computeLadder(
  ...
  const listResult = computeStage(basePrice, list);
  const tradeResult = computeStage(listResult.price, trade);
  const mrpResult = computeStage(tradeResult.price, mrp);
